...
export default {
  ...
  mounted() {
    window.addEventListener('resize', this.updateResponsiveWidth);
  },
  beforeDestroy() {
    window.removeEventListener('resize', this.updateResponsiveWidth);
  },
  methods: {
    goToLogin() {
    ...
    updateResponsiveWidth() {
      this.responsiveWidth.width = window.innerWidth < 768 ? '95%' : '85%';
    },
  },
};
</script>
